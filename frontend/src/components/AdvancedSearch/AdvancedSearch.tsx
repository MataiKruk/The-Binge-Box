...
const AdvancedSearch = () => {
  ...
  const [releaseDateAfter, setReleaseDateAfter] =
    useState<string>("1800-01-01");
  const [releaseDateBefore, setReleaseDateBefore] =
    useState<string>("2050-01-01");
  const [sortBy, setSortBy] = useState<string>("title.asc");
  const [moviesReturned, setMoviesReturned] = useState<boolean>(
  ...
  const handleNextPage = () => {
    setPageNumb((pageNumb) => pageNumb + 1);
  };

  const handlePrevPage = () => {
    if (pageNumb > 1) {
      setPageNumb((pageNumb) => (pageNumb > 1 ? pageNumb - 1 : pageNumb));
    }
  };
    ...
    <div className="advanced-search-body">
      ...
      <div className="page-info-advanced-search">
        <button
          ...
          disabled={pageNumb < 2 ? true : false}
        >
      ...
      {isSearching && moviesReturned && (
        <div className="after-search-display">
          <div className="filteredMoviesContainer">
            <h2 className="advanced-search-search-results-title">
              Search Results:
            </h2>
